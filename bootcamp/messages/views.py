...
def messages(request, username):
    ...
    try:
        ...
        # busca si el usuario existe o no, en caso que modifique manualmente la url
        if username:
            conversations = Message.get_conversations(user=request.user)
        ...
        # sino existe el usuario lo redirecciona a los mensajes
        return redirect(u'/messages/')
...
def new(request):
    if request.method == 'POST':
        ...
        conversations = Message.get_conversations(user=request.user)
        return render(request, 'messages/new.html', {'conversations': conversations,'usuario':request.user})
...
def send(request):
    if request.method == 'POST':
        ...
        if len(message.strip()) == 0:
            return HttpResponse()
            # el usuario no puede enviarse mensajes a si mismo
        if from_user != to_user:
            msg = Message.send_message(from_user, to_user, message)
            return render(request, 'messages/includes/partial_message.html', {'message': msg,'usuario':request.user})
        return HttpResponse()
    else:
...
def users(request):
    ...
    template = u'{0} ({1})'
    for user in users:
        # El usuario no se puede mandar mensajes a si mismo.
